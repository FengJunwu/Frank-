...
//菜单的显示区域占屏幕宽度的百分比
static CGFloat MenuWidthScale = 0.73f;
//遮罩层最高透明度
...
-(void)setLeftViewController:(UIViewController *)leftViewController{
    ...
    _leftViewController.view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kMainScreenWidth, self.view.bounds.size.height)];
...
- (void)showLeftViewControllerAnimated:(BOOL)animated {
    ...
    [_rootViewController.view bringSubviewToFront:_coverView];
    [UIView animateWithDuration:[self animationDurationAnimated:animated] animations:^{
        _rootViewController.view.frame = CGRectMake([self menuWidth], AAdaption(230), kMainScreenWidth, kMainScreenHeight - AAdaption(553));
//        _rootViewController.view.center = CGPointMake(_rootViewController.view.bounds.size.width/2 + self.menuWidth, _rootViewController.view.center.y);

        _leftViewController.view.frame = CGRectMake(0, 0, kMainScreenWidth, self.view.bounds.size.height);
        _coverView.frame = _rootViewController.view.frame;
