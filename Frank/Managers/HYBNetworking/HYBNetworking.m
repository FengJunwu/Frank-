...
static NSTimeInterval sg_timeout = 5.0f;
...
+ (HYBURLSessionTask *)_requestWithUrl:(NSString *)url
  ...
  if (httpMethod == 1) {
    ...
    session = [SessionManager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
      if (progress) {
    ...
    session = [SessionManager POST:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
      if (progress) {
        progress(downloadProgress.completedUnitCount, downloadProgress.totalUnitCount);
      }
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
+ (HYBURLSessionTask *)uploadFileWithUrl:(NSString *)url
  ...
  [SessionManager uploadTaskWithRequest:request fromFile:[NSURL URLWithString:uploadingFile] progress:^(NSProgress * _Nonnull uploadProgress) {
    if (progress) {
...
+ (HYBURLSessionTask *)uploadWithImage:(UIImage *)image
  ...
  HYBURLSessionTask *session = [SessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (HYBURLSessionTask *)downloadWithUrl:(NSString *)url
  ...
  session = [SessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
    if (progressBlock) {
...
+ (HYBURLSessionTask *)uploadWithImagesArray:(NSMutableArray *)images url:(NSString *)url filename:(NSString *)filename names:(NSMutableArray *)names mimeType:(NSString *)mimeType parameters:(NSDictionary *)parameters progress:(HYBUploadProgress)progress success:(HYBResponseSuccess)success fail:(HYBResponseFail)fail{
    ...
    HYBURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (int j = 0;j < images.count;j++) {
            ...
            NSString *imageFileName = filename;
            imageFileName = [NSString stringWithFormat:@"%@%d.png",dateString,i];
            NSData *imageData;
            
            imageData = UIImageJPEGRepresentation(images[j], 0.5f);
            
            // 上传图片，以文件流的格式
            if (mimeType == nil) {
                [formData appendPartWithFileData:imageData name:names[j] fileName:imageFileName mimeType:@"image/jpg"];
            }else{
                
                [formData appendPartWithFileData:imageData name:names[j] fileName:imageFileName mimeType:mimeType];
            }
        }
...
+ (void)ba_uploadFileWithUrlString:(NSString *)urlString
                        parameters:(NSDictionary *)parameters
                              name:(NSString*)name
                          fileName:(NSString*)fileName
                          filePath:(NSString*)filePath
                          mineType:(NSString*)mineType
                  withSuccessBlock:(HYBResponseSuccess)successBlock
                  withFailureBlock:(HYBResponseFail)failureBlock
                withUploadProgress:(HYBPostProgress)progress
{
    if ([self baseUrl] == nil) {
    ...
    [manager POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //获得沙盒中的视频内容
        [formData appendPartWithFileURL:[NSURL URLWithString:filePath] name:name fileName:fileName mimeType:mineType error:nil];
        
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        
        HYBAppLog(@"上传进度--%lld,总进度---%lld",uploadProgress.completedUnitCount,uploadProgress.totalUnitCount);
