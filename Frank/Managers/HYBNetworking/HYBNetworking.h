 ...
 *
 *    上传传多张图片接口，若不指定baseurl，可传完整的url
 *
 *    @param images            图片对象
 *    @param url                上传图片的接口路径，如/path/images/
 *    @param filename        给图片起一个名字，默认为当前日期时间,格式为"yyyyMMddHHmmss"，后缀为`jpg`
 ...
 */
+ (HYBURLSessionTask *)uploadWithImagesArray:(NSMutableArray *)images
                                         url:(NSString *)url
                                    filename:(NSString *)filename
...
/**
 ...
 @param urlString url
 @param parameters 参数
 @param name 与服务端约定的参数
 @param fileName 文件名称
 @param filePath 本地文件路径
 @param mineType 文件格式类型 [mp3 : application/octer-stream application/octet-stream] [mp4 : video/mp4]
